...
# Experiment controls
# --------------------------------------------------
# This script uses TRAINING-ONLY augmentation.
# Validation and test samples stay clean/fair.
USE_AUGMENTATION = True
AUGMENT_COPIES_PER_TRAIN_SAMPLE = 1
AUGMENT_LABELS = ["unknown", "flying", "happy"]  # Do not augment background for this run
...
def augment_audio(audio):
    """
    Simple training-only audio augmentation:
    - random volume scaling
    - random time shift

    This function should only be applied to training clips.
    """
    audio = audio.astype(np.float32).copy()

    # Random volume scaling
    if np.random.rand() < 0.75:
        gain = np.random.uniform(0.70, 1.30)
        audio = audio * gain

    # Random time shift: +/- 0.10 seconds at 16 kHz
    if np.random.rand() < 0.75:
        max_shift = int(0.10 * SAMPLE_RATE)
        shift = np.random.randint(-max_shift, max_shift + 1)
        audio = np.roll(audio, shift)

        # Zero-fill the wrapped part so audio does not loop around
        if shift > 0:
            audio[:shift] = 0.0
        elif shift < 0:
            audio[shift:] = 0.0
...
def raw_audio_to_spectrogram_array(raw_audio_array):
    specs = []
    for audio in raw_audio_array:
        specs.append(make_spectrogram(audio))
...
def build_raw_dataset():
    """
    Builds raw audio arrays first instead of spectrograms.
    This lets us split into train/val/test before augmentation.
    Augmentation is then applied only to the training split.
    """
    X_raw = []
    y = []
    ...
    return X_raw, y, dataset_file_counts, dataset_sample_counts


def augment_training_split(X_train_raw, y_train):
    """
    Keeps all original training samples and adds augmented copies only for selected speech labels.
    Validation and test are not augmented.
    """
    if not USE_AUGMENTATION:
        augmentation_counts = {label: 0 for label in LABELS}
        return X_train_raw, y_train, augmentation_counts

    print("\nApplying training-only augmentation...")
    print(f"Augmentation labels: {AUGMENT_LABELS}")
    print(f"Augmented copies per selected training sample: {AUGMENT_COPIES_PER_TRAIN_SAMPLE}")
    ...
    label_to_index = {label: i for i, label in enumerate(LABELS)}
    augment_indices = {label_to_index[label] for label in AUGMENT_LABELS}

    for audio, label_idx in zip(X_train_raw, y_train):
        # Keep original training sample
        X_aug.append(audio)
        y_aug.append(label_idx)

        # Add augmented copies only for selected classes
        if int(label_idx) in augment_indices:
            label_name = LABELS[int(label_idx)]
            for _ in range(AUGMENT_COPIES_PER_TRAIN_SAMPLE):
                aug_audio = augment_audio(audio)
                X_aug.append(aug_audio)
                y_aug.append(label_idx)
                augmentation_counts[label_name] += 1

    X_aug = np.array(X_aug, dtype=np.float32)
    y_aug = np.array(y_aug, dtype=np.int64)

    print("\nTraining split after augmentation:")
    print("X_train_raw_aug shape:", X_aug.shape)
    print("y_train_aug shape:    ", y_aug.shape)

    print("\nAdded augmented training samples:")
    for label in LABELS:
        print(f"{label:12s}: {augmentation_counts[label]} added")

    return X_aug, y_aug, augmentation_counts
...
def save_run_summary(
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        ...
        f.write("-" * 70 + "\n")
        f.write(f"Augmentation enabled: {USE_AUGMENTATION}\n")
        f.write("Augmentation methods: random volume scaling, random time shift\n")
        f.write(f"Augment labels: {AUGMENT_LABELS}\n")
        f.write(f"Augment copies per selected training sample: {AUGMENT_COPIES_PER_TRAIN_SAMPLE}\n")
        f.write("Augmentation was applied only to the training split. Validation and test were clean.\n")
        f.write("Augmented samples added to training split:\n")
        for label in LABELS:
            f.write(f"{label}: {augmentation_counts[label]} added\n")
        f.write("\n")
        ...
        f.write("\nDataset sample counts before augmentation\n")
        f.write("-" * 70 + "\n")
        for label in LABELS:
...
def save_config():
    config = {
        ...
        "augmentation_methods": ["random_volume_scaling", "random_time_shift"],
        "augmentation_labels": AUGMENT_LABELS,
        "augment_copies_per_train_sample": AUGMENT_COPIES_PER_TRAIN_SAMPLE,
        "augmentation_applied_to": "training_split_only",
    }
...
def main():
    ...
    X_raw, y, dataset_file_counts, dataset_sample_counts = build_raw_dataset()

    # Split raw audio into train, validation, and test BEFORE augmentation.
    # This prevents augmented versions of a test/val sample from leaking into training.
    ...
    # Apply augmentation only to training audio.
    X_train_raw_aug, y_train_aug, augmentation_counts = augment_training_split(X_train_raw, y_train)

    print("\nConverting raw audio to spectrograms...")
    X_train = raw_audio_to_spectrogram_array(X_train_raw_aug)
    X_val = raw_audio_to_spectrogram_array(X_val_raw)
    X_test = raw_audio_to_spectrogram_array(X_test_raw)
    y_train = y_train_aug
    ...
    print("\nSplit sizes after training-only augmentation and spectrogram conversion:")
